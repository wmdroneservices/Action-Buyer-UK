document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function load() {
    ...
    root.innerHTML=`
      <div class="valuation-card"><div style="display:flex;justify-content:space-between;gap:1rem;align-items:flex-start;flex-wrap:wrap"><div><p class="section-kicker"${status==='Repair Required'?' style="display:inline-block;color:#b42318;background:#fff1f0;border:1px solid #b42318;padding:.4rem .7rem;border-radius:4px;font-weight:900;letter-spacing:.08em"':''}>PRODUCT WORKBENCH · ${esc(status)}</p><h2>${esc([asset.manufacturer,asset.model].filter(Boolean).join(' ')||'Unnamed asset')}</h2><p>Asset ${esc(asset.asset_reference)} · Transaction ${esc(asset.transaction_number||'Not recorded')}</p></div><div style="display:flex;gap:.5rem;flex-wrap:wrap"><a class="btn btn-secondary" href="inventory.html">BACK TO INVENTORY</a><a class="btn btn-secondary" href="#product-history">PRODUCT HISTORY</a></div></div><div style="display:flex;gap:.4rem;flex-wrap:wrap;margin-top:1rem"><span class="notice"><strong>1</strong> Customer quote</span>${status==='Repair Required'?'<span class="notice"><strong>2</strong> Repair required</span>':''}<span class="notice"><strong>${2+stepOffset}</strong> Inspection & testing</span><span class="notice"><strong>${3+stepOffset}</strong> Photos & package</span><span class="notice"><strong>${4+stepOffset}</strong> Send to Sales</span></div></div>
      ...
      <section id="sales-handoff-section" class="valuation-card" style="margin-top:1rem"><h2>${4+stepOffset}. Complete & send to Sales</h2>${sourceSale&&!purchaseFinalised?'<div class="notice" style="border-left-color:#d88732"><strong>PURCHASING NOT FINALISED</strong><p>The physical inspection is complete, but this customer purchase cannot enter Sales until the final offer is accepted and payment is completed.</p></div>':''}<div class="notice"><strong>Completion gate</strong><ul><li>Customer quote checked against item received.</li><li>Staff condition recorded.</li><li>Serial and battery counts checked where applicable.</li><li>Missing items resolved and final package contents recorded.</li><li>Technical tests completed.</li><li>Resale description is usable.</li></ul></div><div style="display:flex;gap:.6rem;flex-wrap:wrap;margin-top:1rem"><a class="btn btn-secondary" href="inventory.html">SAVE & RETURN TO INVENTORY</a>${canSend?'<button id="send-sales" class="btn btn-primary" type="button">SEND TO SALES</button>':(!purchaseFinalised?'<button class="btn btn-primary" type="button" disabled>WAITING FOR PURCHASE FINALISATION</button>':'<button class="btn btn-primary" type="button" disabled>SEND TO SALES — COMPLETE WORKFLOW FIRST</button>')}</div>${!canSend?'<p class="form-message error">Blocked: current status is '+esc(status)+'. '+(status==='Repair Required'?'Complete and record the repair. Repair completion marks the post-repair test as passed and releases the item to Ready for Resale; then complete any remaining package or condition requirements.':'Complete the missing inspection, testing, package and condition requirements shown above.')+'</p>':''}<p id="send-message" class="form-message" aria-live="polite"></p></section>`;
    ...
    root.querySelector('#workbench-form').addEventListener('submit',async e=>{
      ...
      m.textContent='Product, inspection, testing, package, price and master description saved.';m.className='form-message success';b.disabled=false;setTimeout(load,450);
    });
