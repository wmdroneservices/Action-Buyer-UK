document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function load() {
    ...
    root.innerHTML=`
      <div class="valuation-card"><div style="display:flex;justify-content:space-between;gap:1rem;align-items:flex-start;flex-wrap:wrap"><div><p class="section-kicker"${status==='Repair Required'?' style="display:inline-block;color:#b42318;background:#fff1f0;border:1px solid #b42318;padding:.4rem .7rem;border-radius:4px;font-weight:900;letter-spacing:.08em"':''}>PRODUCT WORKBENCH · ${esc(status)}</p><h2>${esc([asset.manufacturer,asset.model].filter(Boolean).join(' ')||'Unnamed asset')}</h2><p>Asset ${esc(asset.asset_reference)} · Transaction ${esc(asset.transaction_number||'Not recorded')}</p></div><div style="display:flex;gap:.5rem;flex-wrap:wrap"><a class="btn btn-secondary" href="inventory.html">BACK TO INVENTORY</a>${['Sent to Sales','Listed','Reserved'].includes(status)?`<a class="btn btn-primary" href="listing-readiness.html?id=${encodeURIComponent(id)}">OPEN SALES WORKBENCH</a>`:''}</div></div><div style="display:flex;gap:.4rem;flex-wrap:wrap;margin-top:1rem"><span class="notice"><strong>1</strong> Customer quote</span>${status==='Repair Required'?'<span class="notice"><strong>2</strong> Repair required</span>':''}<span class="notice"><strong>${2+stepOffset}</strong> Inspection & testing</span><span class="notice"><strong>${3+stepOffset}</strong> Photos & package</span><span class="notice"><strong>${4+stepOffset}</strong> Send to Sales</span></div></div>
