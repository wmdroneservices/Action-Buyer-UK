document.addEventListener("DOMContentLoaded", function () {
  ...
  function currentStep() {
    return Array.from(form.querySelectorAll(".wizard-step")).find(s => !s.hidden);
  }
  function nonDJI() {
    const category = document.getElementById("gear-category");
    const manufacturer = document.getElementById("gear-manufacturer");
    return category && manufacturer && !(category.value === "drone" && manufacturer.value === "DJI");
  }
  function text(id) {
  ...
  function show(stepNo) {
    form.querySelectorAll(".wizard-step").forEach(s => { s.hidden = Number(s.dataset.step) !== stepNo; });
  ...
  function render() {
    ...
    step.querySelectorAll("#quote-important, .quote-important, #quote-result-action, .btn-accept").forEach(el => el.remove());
    const title = document.getElementById("quote-result-title");
    if (title) title.textContent = "Manual Valuation Required";
    const category = text("gear-category");
    const manufacturer = text("gear-manufacturer");
    const model = text("dji-model");
    const exists = window.__gearCashOutItems.some(i => i.model === model && i.manufacturer === manufacturer);
    if (!exists) window.__gearCashOutItems.push({ category, manufacturer, model });
  ...
  form.addEventListener("click", function (event) {
    ...
    if (!button || !nonDJI()) return;
    const step = currentStep();
    if (!step) return;
    const n = Number(step.dataset.step);
    if (n === 12 && button.id === "add-another-item") {
      event.preventDefault(); event.stopImmediatePropagation();
      form.querySelectorAll(".wizard-step").forEach(s => { s.hidden = true; });
      form.querySelector('[data-step="1"]').hidden = false;
      document.getElementById("gear-category").value = "";
      document.getElementById("gear-manufacturer").innerHTML = '<option value="">-- Select manufacturer --</option>';
      document.getElementById("gear-manufacturer").disabled = true;
      document.getElementById("dji-model").innerHTML = '<option value="">-- Select a model --</option>';
      return;
    }
    if (n === 12 && button.id === "continue-with-quote") {
      event.preventDefault(); event.stopImmediatePropagation();
      show(13); return;
    }
    if (n === 13 && button.classList.contains("btn-next")) {
      event.preventDefault(); event.stopImmediatePropagation();
  ...
  const result = form.querySelector('[data-step="12"]');
  if (result) new MutationObserver(function () {
    if (!result.hidden && nonDJI() && /£0\.00|Select package|Your Instant Quote/.test(result.textContent)) render();
