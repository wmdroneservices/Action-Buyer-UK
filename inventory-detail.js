document.addEventListener("DOMContentLoaded", async () => {
  ...
  container.innerHTML = `<div class="valuation-card"><p class="section-kicker">${esc(asset.status)}</p><h2>${esc([asset.manufacturer, asset.model].filter(Boolean).join(" "))}</h2><p>Asset reference: ${esc(asset.asset_reference)}</p><p>Serial number: ${esc(asset.serial_number || "Not recorded")}</p><hr><p><strong>Purchase price:</strong> ${money(asset.purchase_price)}</p><p><strong>Condition:</strong> ${esc(asset.condition_grade || "Not recorded")}</p><p><strong>Location:</strong> ${esc(asset.current_location || "Not recorded")}</p><p><strong>Notes:</strong> ${esc(asset.notes || "No notes")}</p></div>`;
});
