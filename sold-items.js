document.addEventListener('DOMContentLoaded', async () => {
  ...
  if(delistCount){
    ...
    urgent.innerHTML=`<div style="font-size:.8rem;font-weight:900;letter-spacing:.12em;color:#b42318">URGENT — CLOSE OTHER MARKETPLACE LISTINGS</div><h2 style="margin:.3rem 0;color:#7f1d1d">${delistCount} LISTING${delistCount===1?'':'S'} REQUIRE IMMEDIATE ACTION</h2><p style="margin:.25rem 0 .9rem;color:#5f1b18"><strong>A product has already sold.</strong> Use the dedicated closure page to review the exact SKU, the affected listing and all recorded channels for the same item.</p><div style="display:grid;gap:.75rem">${delistRows.map(l=>{
