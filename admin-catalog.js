...
function evidenceKey(r){return [String(r.retailer||'').trim().toLowerCase(),String(r.price_type||'').trim().toLowerCase(),String(r.condition||'').trim().toLowerCase(),r.buy_price??'',r.sell_price??'',r.reference_price_max??'',safeUrl(r.source_url||'').replace(/\/$/,'').toLowerCase()].join('|');}
...
function renderRetailers(){
  ...
  body.innerHTML=retailerRows.map((r,i)=>{const url=safeUrl(r.source_url);return `<tr data-index="${i}"${r.id?` data-id="${esc(r.id)}"`:''}>
    ...
    <td><input class="retailer-sell" type="number" min="0" step="0.01" value="${r.reference_price_min??r.sell_price??''}" placeholder="From / selling"></td>
    <td><input class="retailer-sell-max" type="number" min="0" step="0.01" value="${r.reference_price_max??''}" placeholder="To (range only)"></td>
...
function filteredProducts(){
  const q=val('search').toLowerCase(),manufacturer=val('manufacturer-filter').toLowerCase(),category=val('category-filter').toLowerCase(),status=val('status-filter').toLowerCase(),pricing=val('pricing-filter').toLowerCase();
  return products.filter(p=>{const textMatch=!q||[p.category,p.manufacturer,p.model,p.package_key,p.package_name].some(v=>String(v||'').toLowerCase().includes(q));const manufacturerMatch=!manufacturer||String(p.manufacturer||'').toLowerCase()===manufacturer;const categoryMatch=!category||String(p.category||'').toLowerCase()===category;const statusMatch=!status||(status==='active'?!!p.active:!p.active);const priceFields=['factory_sealed_price','opened_unused_price','excellent_price','good_price','fair_price'];const priced=priceFields.filter(k=>p[k]!==null&&p[k]!==undefined&&p[k]!=='').length;const source=(p.pricing_source||'').toLowerCase();const pricingMatch=!pricing||(pricing==='automatic'?source==='automatic':pricing==='manual'?source==='manual':pricing==='any'?priced>0:pricing==='none'?priced===0:priced>0&&priced<5);return textMatch&&manufacturerMatch&&categoryMatch&&statusMatch&&pricingMatch;});
}
...
function renderCatalogueStatusSummary(){
  const list=$('catalogue-status-summary-list');if(!list)return;
  const groups=new Map();
  products.forEach(p=>{
    const manufacturer=String(p.manufacturer||'Unbranded').trim()||'Unbranded';
    const category=String(p.category||'Other').trim()||'Other';
    const key=manufacturer+'\u0000'+category;
    if(!groups.has(key))groups.set(key,{manufacturer,category,active:0,inactive:0});
    const group=groups.get(key);
    if(p.active)group.active++;else group.inactive++;
  });
  const rows=[...groups.values()].sort((a,b)=>a.manufacturer.localeCompare(b.manufacturer)||a.category.localeCompare(b.category));
  if(!rows.length){list.innerHTML='<div class="catalogue-status-summary-empty">No catalogue products found.</div>';return;}
  list.innerHTML=rows.map(row=>`<article class="catalogue-status-summary-card"><h3>${esc(row.manufacturer)} · ${esc(row.category)}</h3><div class="catalogue-status-summary-counts"><span class="catalogue-status-summary-count active"><strong>${row.active}</strong> active</span><span class="catalogue-status-summary-count inactive"><strong>${row.inactive}</strong> inactive</span></div></article>`).join('');
}
function renderList(){renderCatalogueStatusSummary();populateFilters();const list=$('catalog-list');if(!list)return;const rows=filteredProducts();const pages=Math.max(1,Math.ceil(rows.length/PAGE_SIZE));if(currentPage>pages)currentPage=pages;if(!rows.length){list.innerHTML='<div class="empty-account"><h3>No products found</h3><p>No catalogue products match the selected filters.</p></div>';renderPagination(0);return;}const pageRows=rows.slice((currentPage-1)*PAGE_SIZE,currentPage*PAGE_SIZE);list.innerHTML=`<div class="valuation-list">${pageRows.map(p=>`<div class="valuation-card"><div><div class="valuation-ref">${esc(p.manufacturer)} · ${esc(p.category||'')}</div><h3>${esc(p.manufacturer)} ${esc(p.model)} — ${esc(p.package_name||p.package_key)}</h3><p>Online comparison — · Sealed ${money(p.factory_sealed_price)} · Unused ${money(p.opened_unused_price)} · Excellent ${money(p.excellent_price)} · Good ${money(p.good_price)} · Fair ${money(p.fair_price)}</p></div><div class="valuation-meta"><span class="status-badge">${p.active?'Active':'Inactive'}</span><button type="button" class="btn btn-secondary edit-product" data-id="${esc(p.id)}">EDIT</button></div></div>`).join('')}</div>`;renderPagination(rows.length);}
function resetToFirstPage(){currentPage=1;renderList();}
function clearFilters(){setVal('search','');setVal('manufacturer-filter','');setVal('category-filter','');setVal('status-filter','');setVal('pricing-filter','');resetToFirstPage();}

async function load(){
  if(!auth||!sb()){showMessage('Authentication system unavailable.',true);return;}
  try{const session=await auth.getSession();if(!session){location.href='login.html?return=admin-catalog.html';return;}const {data:staff,error:staffError}=await sb().from('staff_users').select('user_id').eq('user_id',session.user.id).maybeSingle();if(staffError)throw staffError;if(!staff){document.body.innerHTML='<main class="account-page"><div class="container"><section class="account-panel"><h1>Staff access required</h1><p>This page is restricted to staff accounts.</p></section></div></main>';return;}let allProducts=[],from=0;const pageSize=1000;while(true){const {data,error}=await sb().from('quote_catalog_products').select('id,category,manufacturer,model,package_key,package_name,factory_sealed_price,opened_unused_price,excellent_price,good_price,fair_price,pricing_source,automatic_pricing_rule_id,active,notes,updated_at').range(from,from+pageSize-1).order('manufacturer').order('model').order('package_name');if(error)throw error;allProducts.push(...(data||[]));if(!data||data.length<pageSize)break;from+=pageSize;}products=allProducts;currentPage=1;populateFilters();renderList();const requestedProductId=new URLSearchParams(location.search).get('product');if(requestedProductId){const requested=products.find(p=>String(p.id)===requestedProductId);if(requested){loadProduct(requested);}}}catch(e){console.error(e);$('catalog-list').textContent='Unable to load the catalogue.';showMessage(e.message||String(e),true);}}

$('catalog-form')?.addEventListener('submit',async e=>{e.preventDefault();try{const session=await auth.getSession();if(!session){location.href='login.html?return=admin-catalog.html';return;}const payload={category:val('category'),manufacturer:val('manufacturer'),model:val('model'),package_key:val('package-key'),package_name:val('package-name'),factory_sealed_price:num('factory-sealed'),opened_unused_price:num('opened-unused'),excellent_price:num('excellent'),good_price:num('good'),fair_price:num('fair'),active:$('active')?.checked!==false,notes:val('notes'),updated_at:new Date().toISOString()};if(!payload.category||!payload.manufacturer||!payload.model||!payload.package_key||!payload.package_name){showMessage('Category, manufacturer, model, package key and package name are required for a new product.',true);return;}const id=val('product-id');let saved;if(id){const r=await sb().from('quote_catalog_products').update(payload).eq('id',id).select().single();if(r.error)throw r.error;saved=r.data;}else{const r=await sb().from('quote_catalog_products').insert(payload).select().single();if(r.error)throw r.error;saved=r.data;}readRetailers();if(saved?.id){const existing=await sb().from('quote_catalog_retailer_prices').select('id').eq('catalog_product_id',saved.id);if(existing.error)throw existing.error;const keep=[];for(const r of retailerRows){if(!r.retailer)continue;const row={catalog_product_id:saved.id,retailer:r.retailer,price_type:r.price_type||'market',condition:r.condition,price_currency:r.price_currency||null,evidence_region:r.evidence_region||null,buy_price:r.buy_price,sell_price:r.sell_price,reference_price_min:r.reference_price_min??r.sell_price,reference_price_max:r.reference_price_max??null,reference_conditions:r.reference_conditions??null,reference_units_observed:r.reference_units_observed??null,reference_only:r.reference_only===true,availability_status:r.availability_status||'unknown',buy_method:r.buy_method,source_url:r.source_url,notes:r.notes,checked_at:new Date().toISOString(),updated_at:new Date().toISOString()};if(r.id){const u=await sb().from('quote_catalog_retailer_prices').update(row).eq('id',r.id);if(u.error)throw u.error;keep.push(r.id);}else{const ins=await sb().from('quote_catalog_retailer_prices').insert(row).select('id').single();if(ins.error)throw ins.error;keep.push(ins.data.id);}}const remove=(existing.data||[]).map(x=>x.id).filter(x=>!keep.includes(x));if(remove.length){const d=await sb().from('quote_catalog_retailer_prices').delete().in('id',remove);if(d.error)throw d.error;}}window.dispatchEvent(new CustomEvent('gco:evidence-saved',{detail:{productId:saved.id}}));showMessage('Product and online comparison evidence saved.');await load();const fresh=products.find(p=>p.id===saved.id);if(fresh)loadProduct(fresh);}catch(e){console.error(e);showMessage(e.message||String(e),true);}});

document.addEventListener('click',e=>{const edit=e.target.closest('.edit-product');if(edit){const p=products.find(x=>x.id===edit.dataset.id);if(p)loadProduct(p);}if(e.target.closest('#clear-form'))clearForm();if(e.target.closest('#clear-filters'))clearFilters();if(e.target.closest('#add-retailer-price')){readRetailers();retailerRows.push({retailer:'',price_type:'new',condition:'New',price_currency:'GBP',evidence_region:'UK',buy_price:null,sell_price:null,reference_price_min:null,reference_price_max:null,availability_status:'in_stock',buy_method:'',source_url:'',notes:'',checked_at:null});renderRetailers();}if(e.target.closest('.remove-retailer')){readRetailers();const tr=e.target.closest('tr');retailerRows.splice(Number(tr.dataset.index),1);renderRetailers();}if(e.target.closest('.catalog-page-prev')){currentPage=Math.max(1,currentPage-1);renderList();window.scrollTo({top:document.getElementById('catalog-list')?.offsetTop||0,behavior:'smooth'});}if(e.target.closest('.catalog-page-next')){const total=filteredProducts().length;const pages=Math.ceil(total/PAGE_SIZE);currentPage=Math.min(pages,currentPage+1);renderList();window.scrollTo({top:document.getElementById('catalog-list')?.offsetTop||0,behavior:'smooth'});}});
$('search')?.addEventListener('input',resetToFirstPage);$('manufacturer-filter')?.addEventListener('change',resetToFirstPage);$('category-filter')?.addEventListener('change',resetToFirstPage);$('status-filter')?.addEventListener('change',resetToFirstPage);$('pricing-filter')?.addEventListener('change',resetToFirstPage);if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{window.setTimeout(load,0);},{once:true});else window.setTimeout(load,0);