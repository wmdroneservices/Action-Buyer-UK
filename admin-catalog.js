document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function fill(r){
    ...
    document.getElementById("package-key").value=r.package_key||"standard";
    document.getElementById("package-name").value=r.package_name||"Standard Package";
    document.getElementById("factory-sealed").value=r.factory_sealed_price??"";
    document.getElementById("opened-unused").value=r.opened_unused_price??"";
    document.getElementById("excellent").value=r.excellent_price??"";
    document.getElementById("good").value=r.good_price??"";
    document.getElementById("fair").value=r.fair_price??"";
    document.getElementById("active").checked=!!r.active;
    document.getElementById("notes").value=r.notes||"";
    await loadRetailerPrices(r.id);
  ...
  function clear(){
    ...
    document.getElementById("package-key").value="standard";
    document.getElementById("package-name").value="Standard Package";
  ...
  function render(){
    ...
    const data=rows.filter(r=>!q||[r.category,r.manufacturer,r.model,r.package_name].some(v=>String(v||"").toLowerCase().includes(q)));
    if(!data.length){list.innerHTML="<p>No catalogue products found.</p>";return;}
    list.innerHTML=data.map(r=>`<article class="valuation-card" style="margin:.75rem 0;"><div><span class="valuation-ref">${esc(r.manufacturer)} ${esc(r.model)}</span><p>${esc(r.category)} · ${esc(r.package_name)} ${r.active?"":"· INACTIVE"}</p><p>Sealed ${money(r.factory_sealed_price)} · Unused ${money(r.opened_unused_price)} · Excellent ${money(r.excellent_price)} · Good ${money(r.good_price)} · Fair ${money(r.fair_price)}</p><p style="font-size:.9rem">${retailerSummary(r.id)}</p>${retailerQuickLinks(r.id)?`<p>${retailerQuickLinks(r.id)}</p>`:""}</div><div><button class="btn btn-secondary edit" data-id="${r.id}" type="button">EDIT</button></div></article>`).join("");
    list.querySelectorAll(".edit").forEach(b=>b.onclick=()=>{const r=rows.find(x=>x.id===b.dataset.id);if(r)fill(r);});
  }

  async function load(){
    const {data,error}=await auth.supabase.from("quote_catalog_products").select("*").order("manufacturer").order("model").order("package_name");
    if(error){setMsg(error.message,false);return;}
    rows=data||[];
    const {data:retailers,error:retailerError}=await auth.supabase.from("quote_catalog_retailer_prices").select("catalog_product_id,retailer,condition,buy_price,sell_price,source_url,checked_at").order("retailer").order("condition");
    if(retailerError){setMsg(retailerError.message,false);retailerRows=[];}else retailerRows=retailers||[];
    render();
  }
  ...
  form.onsubmit=async e=>{
    ...
    const value=(field,elementId)=>{
      const entered=document.getElementById(elementId).value.trim();
      if(entered!=="")return entered;
      return existing?.[field]??"";
    };
    if(!id){
      for(const [field,elementId] of [["category","category"],["manufacturer","manufacturer"],["model","model"]]){
        if(!value(field,elementId)){setMsg(`${field.charAt(0).toUpperCase()+field.slice(1)} is required for a new product.`,false);document.getElementById(elementId).focus();return;}
      }
    }
    const payload={
      p_id:id,
      p_category:value("category","category"),
      p_manufacturer:value("manufacturer","manufacturer"),
      p_model:value("model","model"),
      p_package_key:value("package_key","package-key")||"standard",
      p_package_name:value("package_name","package-name")||"Standard Package",
      p_factory_sealed_price:num("factory-sealed"),
      p_opened_unused_price:num("opened-unused"),
      p_excellent_price:num("excellent"),
      p_good_price:num("good"),
      p_fair_price:num("fair"),
      p_active:document.getElementById("active").checked,
      p_notes:document.getElementById("notes").value
    };
    const {data,error}=await auth.supabase.rpc("staff_upsert_catalog_product",payload);
    if(error){setMsg(error.message,false);return;}
    const productId=payload.p_id||data?.id||data;
    if(!productId){setMsg("Product saved, but its ID could not be resolved for retailer data.",false);await load();return;}
    try{await saveRetailerPrices(productId);}catch(err){setMsg("Product saved, but retailer comparison data could not be saved: "+err.message,false);await load();return;}
    setMsg("Product and retailer comparison data saved.");
    clear();
    await load();
  };

  document.getElementById("clear-form").onclick=clear;
  document.getElementById("add-retailer-price").onclick=()=>addRetailerRow();
