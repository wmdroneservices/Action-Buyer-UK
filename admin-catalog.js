...
function renderList(){populateFilters();const list=$('catalog-list'),q=val('search').toLowerCase(),manufacturer=val('manufacturer-filter').toLowerCase(),category=val('category-filter').toLowerCase();const rows=products.filter(p=>{const textMatch=!q||[p.category,p.manufacturer,p.model,p.package_key,p.package_name].some(v=>String(v||'').toLowerCase().includes(q));return textMatch&&(!manufacturer||String(p.manufacturer||'').toLowerCase()===manufacturer)&&(!category||String(p.category||'').toLowerCase()===category);});if(!rows.length){list.innerHTML='<div class="empty-account"><h3>No products found</h3><p>No catalogue products match the selected filters.</p></div>';return;}list.innerHTML=`<div class="valuation-list">${rows.map(p=>`<div class="valuation-card"><div><div class="valuation-ref">${esc(p.manufacturer)} · ${esc(p.category||'')}</div><h3>${esc(p.manufacturer)} ${esc(p.model)} — ${esc(p.package_name||p.package_key)}</h3><p>Sealed ${money(p.factory_sealed_price)} · Unused ${money(p.opened_unused_price)} · Excellent ${money(p.excellent_price)} · Good ${money(p.good_price)} · Fair ${money(p.fair_price)}</p></div><div class="valuation-meta"><span class="status-badge">${p.active?'Active':'Inactive'}</span><button type="button" class="btn btn-secondary edit-product" data-id="${p.id}">EDIT</button></div></div>`).join('')}</div>`;}
function clearFilters(){setVal('search','');setVal('manufacturer-filter','');setVal('category-filter','');renderList();}

async function load(){if(!auth||!sb()){showMessage('Authentication system unavailable.',true);return;}try{const session=await auth.getSession();if(!session){location.href='login.html?return=admin-catalog.html';return;}const {data:staff,error:staffError}=await sb().from('staff_users').select('user_id').eq('user_id',session.user.id).maybeSingle();if(staffError)throw staffError;if(!staff){document.body.innerHTML='<main class="account-page"><div class="container"><section class="account-panel"><h1>Staff access required</h1><p>This page is restricted to staff accounts.</p></section></div></main>';return;}let allProducts=[],from=0;const pageSize=1000;while(true){const {data,error}=await sb().from('quote_catalog_products').select('id,category,manufacturer,model,package_key,package_name,manufacturer_rrp,manufacturer_rrp_currency,manufacturer_rrp_source,manufacturer_rrp_verified_at,factory_sealed_price,opened_unused_price,excellent_price,good_price,fair_price,active,notes,updated_at').range(from,from+pageSize-1).order('manufacturer').order('model').order('package_name');if(error)throw error;allProducts.push(...(data||[]));if(!data||data.length<pageSize)break;from+=pageSize;}products=allProducts;populateFilters();renderList();}catch(e){console.error(e);$('catalog-list').textContent='Unable to load the catalogue.';showMessage(e.message||String(e),true);}}
...
document.addEventListener('click',e=>{const edit=e.target.closest('.edit-product');if(edit){const p=products.find(x=>x.id===edit.dataset.id);if(p)loadProduct(p);}if(e.target.closest('#clear-form'))clearForm();if(e.target.closest('#clear-filters'))clearFilters();if(e.target.closest('#add-retailer-price')){readRetailers();retailerRows.push({retailer:'',price_type:'market',condition:'',buy_price:null,sell_price:null,availability_status:'in_stock',buy_method:'',source_url:'',notes:'',checked_at:null});renderRetailers();}if(e.target.closest('.remove-retailer')){readRetailers();const tr=e.target.closest('tr');retailerRows.splice(Number(tr.dataset.index),1);renderRetailers();}});
$('search')?.addEventListener('input',renderList);$('manufacturer-filter')?.addEventListener('change',renderList);$('category-filter')?.addEventListener('change',renderList);document.addEventListener('DOMContentLoaded',load);