...
function renderUkMarketReference(){
  ...
  const groups={};rows.forEach(r=>{const key=String(r.retailer||'Other').trim()||'Other';const n=Number(r.sell_price);if(Number.isFinite(n))groups[key]=groups[key]===undefined?n:Math.max(groups[key],n);});
  const cards=[];Object.entries(groups).slice(0,6).forEach(([name,price])=>cards.push(`<div class="uk-market-stat"><strong>${esc(name)} ${money(price)}</strong><span>UK selling-price reference</span></div>`));
...
function renderRetailers(){
  ...
  if(!retailerRows.length){body.innerHTML='<tr><td colspan="11">No market evidence recorded yet. Add every relevant UK price found, including sale prices, regular prices, used prices, buying prices and out-of-stock prices.</td></tr>';return;}
  body.innerHTML=retailerRows.map((r,i)=>{const url=safeUrl(r.source_url);return `<tr data-index="${i}"${r.id?` data-id="${esc(r.id)}"`:''}>
    ...
    <td><input class="retailer-notes" value="${esc(r.notes)}" placeholder="Sale/offer, was £X, OOS, package, evidence"></td>
...
function clearForm(){
  $('catalog-form')?.reset();setVal('product-id','');setVal('manufacturer-rrp','');setVal('manufacturer-rrp-currency','GBP');setVal('manufacturer-rrp-source','');retailerRows=[];renderRetailers();$('catalog-message').textContent='';$('catalog-message').className='form-message';if($('active'))$('active').checked=true;
}

function loadProduct(p){
  setVal('product-id',p.id);setVal('category',p.category);setVal('manufacturer',p.manufacturer);setVal('model',p.model);setVal('package-key',p.package_key);setVal('package-name',p.package_name);setVal('manufacturer-rrp',p.manufacturer_rrp);setVal('manufacturer-rrp-currency',p.manufacturer_rrp_currency||'GBP');setVal('manufacturer-rrp-source',p.manufacturer_rrp_source||'');setVal('factory-sealed',p.factory_sealed_price);setVal('opened-unused',p.opened_unused_price);setVal('excellent',p.excellent_price);setVal('good',p.good_price);setVal('fair',p.fair_price);setVal('notes',p.notes||'');if($('active'))$('active').checked=!!p.active;
...
function filteredProducts(){
  ...
  return products.filter(p=>{
    ...
    const statusMatch=!status||(status==='active'?!!p.active:!p.active);
    return textMatch&&manufacturerMatch&&categoryMatch&&statusMatch;
  });
}

function ensurePagination(){
  let el=$('catalog-pagination');
  if(!el){el=document.createElement('div');el.id='catalog-pagination';const list=$('catalog-list');list?.parentNode?.insertBefore(el,list.nextSibling);}
  return el;
}

function renderPagination(total){
  const el=ensurePagination();if(!el)return;
  const pages=Math.max(1,Math.ceil(total/PAGE_SIZE));if(currentPage>pages)currentPage=pages;
  ...
  el.style.cssText='display:flex;align-items:center;justify-content:space-between;gap:.75rem;flex-wrap:wrap;margin:1rem 0';
  el.innerHTML=`<span>Showing ${start}–${end} of ${total} products</span><span style="display:flex;gap:.5rem;align-items:center"><button type="button" class="btn btn-secondary catalog-page-prev" ${currentPage===1?'disabled':''}>PREVIOUS</button><strong>Page ${currentPage} of ${pages}</strong><button type="button" class="btn btn-secondary catalog-page-next" ${currentPage===pages?'disabled':''}>NEXT</button></span>`;
}
...
function renderList(){
  ...
  if(!rows.length){list.innerHTML='<div class="empty-account"><h3>No products found</h3><p>No catalogue products match the selected filters.</p></div>';renderPagination(0);return;}
  const pageRows=rows.slice((currentPage-1)*PAGE_SIZE,currentPage*PAGE_SIZE);
  list.innerHTML=`<div class="valuation-list">${pageRows.map(p=>`<div class="valuation-card"><div><div class="valuation-ref">${esc(p.manufacturer)} · ${esc(p.category||'')}</div><h3>${esc(p.manufacturer)} ${esc(p.model)} — ${esc(p.package_name||p.package_key)}</h3><p>Sealed ${money(p.factory_sealed_price)} · Unused ${money(p.opened_unused_price)} · Excellent ${money(p.excellent_price)} · Good ${money(p.good_price)} · Fair ${money(p.fair_price)}</p></div><div class="valuation-meta"><span class="status-badge">${p.active?'Active':'Inactive'}</span><button type="button" class="btn btn-secondary edit-product" data-id="${esc(p.id)}">EDIT</button></div></div>`).join('')}</div>`;
...
async function load(){
  ...
  try{
    ...
    let allProducts=[],from=0;const pageSize=1000;
    while(true){
      ...
      if(error)throw error;allProducts.push(...(data||[]));if(!data||data.length<pageSize)break;from+=pageSize;
    }
    products=allProducts;currentPage=1;populateFilters();renderList();
  }catch(e){console.error(e);$('catalog-list').textContent='Unable to load the catalogue.';showMessage(e.message||String(e),true);}
}

$('catalog-form')?.addEventListener('submit',async e=>{e.preventDefault();try{const session=await auth.getSession();if(!session){location.href='login.html?return=admin-catalog.html';return;}const payload={category:val('category'),manufacturer:val('manufacturer'),model:val('model'),package_key:val('package-key'),package_name:val('package-name'),manufacturer_rrp:num('manufacturer-rrp'),manufacturer_rrp_currency:val('manufacturer-rrp-currency')||'GBP',manufacturer_rrp_source:val('manufacturer-rrp-source'),manufacturer_rrp_verified_at:num('manufacturer-rrp')!==null?new Date().toISOString():null,factory_sealed_price:num('factory-sealed'),opened_unused_price:num('opened-unused'),excellent_price:num('excellent'),good_price:num('good'),fair_price:num('fair'),active:$('active')?.checked!==false,notes:val('notes'),updated_at:new Date().toISOString()};if(!payload.category||!payload.manufacturer||!payload.model||!payload.package_key||!payload.package_name){showMessage('Category, manufacturer, model, package key and package name are required for a new product.',true);return;}const id=val('product-id');let saved;if(id){const r=await sb().from('quote_catalog_products').update(payload).eq('id',id).select().single();if(r.error)throw r.error;saved=r.data;}else{const r=await sb().from('quote_catalog_products').insert(payload).select().single();if(r.error)throw r.error;saved=r.data;}readRetailers();if(saved?.id){const existing=await sb().from('quote_catalog_retailer_prices').select('id').eq('catalog_product_id',saved.id);if(existing.error)throw existing.error;const keep=[];for(const r of retailerRows){if(!r.retailer)continue;const row={catalog_product_id:saved.id,retailer:r.retailer,price_type:r.price_type||'market',condition:r.condition,buy_price:r.buy_price,sell_price:r.sell_price,availability_status:r.availability_status||'unknown',buy_method:r.buy_method,source_url:r.source_url,notes:r.notes,checked_at:new Date().toISOString(),updated_at:new Date().toISOString()};if(r.id){const u=await sb().from('quote_catalog_retailer_prices').update(row).eq('id',r.id);if(u.error)throw u.error;keep.push(r.id);}else{const ins=await sb().from('quote_catalog_retailer_prices').insert(row).select('id').single();if(ins.error)throw ins.error;keep.push(ins.data.id);}}const remove=(existing.data||[]).map(x=>x.id).filter(x=>!keep.includes(x));if(remove.length){const d=await sb().from('quote_catalog_retailer_prices').delete().in('id',remove);if(d.error)throw d.error;}}showMessage('Product and market intelligence saved.');await load();const fresh=products.find(p=>p.id===saved.id);if(fresh)loadProduct(fresh);}catch(e){console.error(e);showMessage(e.message||String(e),true);}});

document.addEventListener('click',e=>{const edit=e.target.closest('.edit-product');if(edit){const p=products.find(x=>x.id===edit.dataset.id);if(p)loadProduct(p);}if(e.target.closest('#clear-form'))clearForm();if(e.target.closest('#clear-filters'))clearFilters();if(e.target.closest('#add-retailer-price')){readRetailers();retailerRows.push({retailer:'',price_type:'market',condition:'',buy_price:null,sell_price:null,availability_status:'in_stock',buy_method:'',source_url:'',notes:'',checked_at:null});renderRetailers();}if(e.target.closest('.remove-retailer')){readRetailers();const tr=e.target.closest('tr');retailerRows.splice(Number(tr.dataset.index),1);renderRetailers();}if(e.target.closest('.catalog-page-prev')){currentPage=Math.max(1,currentPage-1);renderList();window.scrollTo({top:document.getElementById('catalog-list')?.offsetTop||0,behavior:'smooth'});}if(e.target.closest('.catalog-page-next')){const total=filteredProducts().length;const pages=Math.ceil(total/PAGE_SIZE);currentPage=Math.min(pages,currentPage+1);renderList();window.scrollTo({top:document.getElementById('catalog-list')?.offsetTop||0,behavior:'smooth'});}});
