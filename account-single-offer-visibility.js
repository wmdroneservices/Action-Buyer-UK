...
(function(){
  ...
  async function load(){
    ...
    const {data:sales,error:saleError}=saleIds.length?await client.supabase.from("sales").select("id,sale_reference,status,total_amount,created_at,payment_sent_at").in("id",saleIds):{data:[]};
    ...
    for(const v of vals||[]){const key=String(v.quote_data?.submissionKey||v.id);const count=(items||[]).filter(i=>i.valuation_id===v.id).length;groupCounts.set(key,(groupCounts.get(key)||0)+count);}
    const singles=[];
    const finals=[];
    for(const v of vals||[]){
  ...
  function hideInProgressForPublishedOffers(data){
    const valuationsBox=document.getElementById("valuations");
    if(!valuationsBox)return;
    const ids=new Set([...data.singles,...data.finals].map(x=>String(x.v.id)));
    valuationsBox.querySelectorAll(".valuation-card").forEach(card=>{
      const ref=card.querySelector(".valuation-ref")?.textContent?.trim();
      const match=[...data.singles,...data.finals].find(x=>String(x.v.quote_reference||"")===String(ref||""));
      if(match)card.remove();
    });
    if(!valuationsBox.querySelector(".valuation-card")){
      const existingNotice=valuationsBox.querySelector("p");
      if(!existingNotice)valuationsBox.innerHTML="<p>No valuations currently in progress.</p>";
    }
  }
  ...
  async function init(){
    ...
    async function refresh(){try{const data=await load();if(!data)return;renderNewQuotes(data);renderFinalOffers(data);hideInProgressForPublishedOffers(data);}catch(e){console.error("Standalone/final offer visibility:",e);}}
    await refresh();
    setInterval(()=>{if(!document.hidden)refresh();},2000);
    document.addEventListener("click",async event=>{
      const accept=event.target.closest(".gco-final-accept"),refuse=event.target.closest(".gco-final-refuse");
      if(!accept&&!refuse)return;
      event.preventDefault();event.stopImmediatePropagation();
      const button=accept||refuse,offerId=button.dataset.id;if(!offerId)return;
      ...
      if(error){alert(error.message||"The offer response could not be saved.");button.disabled=false;return;}
      lastFinalKey="";window.location.reload();
    },true);
  }
