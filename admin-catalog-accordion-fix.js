/* Catalogue accordion: keep UK pricing separate from overseas evidence and never convert currencies. */
(function(){
  ...
  const isUK=r=>currency(r)==='GBP'&&String(r?.evidence_region||'').trim().toUpperCase()==='UK';
  ...
  function styles(){
    ...
    const s=document.createElement('style');s.id='inline-catalog-style';s.textContent=`
      ...
      .catalog-international-section{margin-top:1.5rem;padding-top:1rem;border-top:3px solid #d88732}.catalog-international-section h3{margin:.1rem 0 .35rem;color:#102f4f}.catalog-international-section p{margin:0 0 .75rem;color:#666;font-size:.88rem}.catalog-international-section table{min-width:1100px}.catalog-international-badge{display:inline-block;margin-left:.4rem;padding:.15rem .45rem;border-radius:999px;background:#f3eee5;color:#102f4f;font-size:.68rem;font-weight:800}.catalog-overseas-price{font-weight:800;white-space:nowrap}.catalog-overseas-currency{font-weight:800;color:#102f4f}.catalog-overseas-empty{padding:.5rem 0;color:#666}
  ...
  function foreignTable(rows,product){
    ...
    const selling=foreign.filter(r=>r.sell_price!==null&&r.sell_price!==undefined&&r.sell_price!=='');
    const buying=foreign.filter(r=>r.buy_price!==null&&r.buy_price!==undefined&&r.buy_price!=='');
    return `<section class="catalog-international-section"><h3>Overseas Comparison (Non-GBP) <span class="catalog-international-badge">ORIGINAL CURRENCY — NO CONVERSION</span></h3><p>These are genuine non-GBP market prices found outside the UK. <strong>Selling prices are shown in the original currency.</strong> They are completely separate from UK Online comparison and never feed the UK price.</p>${selling.length?`<div class="catalog-accordion-market"><table><thead><tr><th>Website / Competitor</th><th>Price type</th><th>Condition</th><th>Currency</th><th>Country / Region</th><th>Overseas selling price</th><th>Buying price</th><th>Availability</th><th>Source</th><th>Notes</th></tr></thead><tbody>${selling.map(r=>{const c=currency(r)||'UNKNOWN';const region=String(r.evidence_region||'').trim()||String(r.price_region||'').trim()||'International';const source=amazonProductUrl(r,product);return `<tr><td>${esc(r.retailer)}</td><td>${esc(String(r.price_type||'').replaceAll('_',' '))}</td><td>${esc(r.condition)}</td><td class="catalog-overseas-currency">${esc(c)}</td><td>${esc(region)}${r.price_region&&r.price_region!==r.evidence_region?`<br><small>${esc(r.price_region)}</small>`:''}</td><td class="catalog-overseas-price">${money(r.sell_price,c)}</td><td>${r.buy_price==null?'—':money(r.buy_price,c)}</td><td>${esc(String(r.availability_status||'').replaceAll('_',' '))}</td><td>${source?`<a href="${esc(source)}" target="_blank" rel="noopener noreferrer">OPEN SOURCE ↗</a>`:'—'}</td><td>${esc(r.notes)}</td></tr>`;}).join('')}</tbody></table></div>`:'<div class="catalog-overseas-empty">No overseas selling price has been recorded for this product yet.</div>'}${buying.length&&!selling.length?`<div class="catalog-accordion-market"><table><thead><tr><th>Website / Competitor</th><th>Price type</th><th>Condition</th><th>Currency</th><th>Country / Region</th><th>Buying price</th><th>Availability</th><th>Source</th><th>Notes</th></tr></thead><tbody>${buying.map(r=>{const c=currency(r)||'UNKNOWN';const region=String(r.evidence_region||'').trim()||String(r.price_region||'').trim()||'International';const source=amazonProductUrl(r,product);return `<tr><td>${esc(r.retailer)}</td><td>${esc(String(r.price_type||'').replaceAll('_',' '))}</td><td>${esc(r.condition)}</td><td class="catalog-overseas-currency">${esc(c)}</td><td>${esc(region)}</td><td class="catalog-overseas-price">${money(r.buy_price,c)}</td><td>${esc(String(r.availability_status||'').replaceAll('_',' '))}</td><td>${source?`<a href="${esc(source)}" target="_blank" rel="noopener noreferrer">OPEN SOURCE ↗</a>`:'—'}</td><td>${esc(r.notes)}</td></tr>`;}).join('')}</tbody></table></div>`:''}</section>`;
  }
