...
(function(){
  ...
  function styles(){
    ...
    const s=document.createElement('style');s.id='inline-catalog-style';s.textContent=`
      ...
      .catalog-active-control{display:inline-flex;align-items:center;justify-content:center;border:0;border-radius:6px;padding:.55rem .8rem;font:inherit;font-size:.78rem;font-weight:800;letter-spacing:.02em;cursor:pointer;vertical-align:middle}.catalog-active-control.active{background:#dff3e4;color:#18733b}.catalog-active-control.inactive{background:#f8dddd;color:#a32323}.catalog-active-control:disabled{opacity:.65;cursor:wait}.catalog-active-control:focus-visible{outline:3px solid #102f4f;outline-offset:2px}
  ...
  async function setActive(card,button){
    ...
    const current=card.dataset.active==='true',desired=!current;button.disabled=true;button.textContent='SAVING…';
    ...
    if(error){button.disabled=false;button.textContent=current?'ACTIVE':'INACTIVE';showError(error.message);return;}
    card.dataset.active=desired?'true':'false';button.disabled=false;button.textContent=desired?'ACTIVE':'INACTIVE';button.classList.toggle('active',desired);button.classList.toggle('inactive',!desired);
  ...
  function wire(){
    ...
    list.addEventListener('click',e=>{
      const activeBtn=e.target.closest('.catalog-active-control');if(activeBtn){e.preventDefault();e.stopPropagation();const card=activeBtn.closest('.catalog-accordion-card');if(card&&!activeBtn.disabled)setActive(card,activeBtn);return;}
