...
(function(){
  ...
  async function setActive(card,button){
    ...
    const current=card.dataset.active==='true';const desired=!current;
    ...
    button.disabled=false;button.textContent=desired?'ACTIVE':'INACTIVE';button.classList.toggle('active',desired);button.classList.toggle('inactive',!desired);
    const status=card.querySelector('.catalog-list-status');if(status){status.textContent=desired?'ACTIVE':'INACTIVE';status.classList.toggle('active',desired);status.classList.toggle('inactive',!desired);}
