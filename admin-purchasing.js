document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function loadCounts() {
    ...
    const sentToSalesSaleIds=new Set((assets||[]).filter(a=>a.source_sale_id&&(String(a.status||"")==="Sent to Sales"||Boolean(a.sent_to_sales_at))).map(a=>a.source_sale_id));
    const purchasingSales=activeSales.filter(s=>!sentToSalesSaleIds.has(s.id));
    const inbound=new Map();
    (shipments||[]).filter(s=>s.shipment_type==="inbound").forEach(s=>{if(!inbound.has(s.sale_id)) inbound.set(s.sale_id,s);});
    const terminal=new Set(["paid","completed","cancelled"]);
    const shippingLabelRequired=purchasingSales.filter(s=>{if(terminal.has(String(s.status||""))) return false;const x=inbound.get(s.id);return !x||["awaiting_label","label_required"].includes(String(x.status||""));});
    const awaitingDelivery=purchasingSales.filter(s=>{if(terminal.has(String(s.status||""))) return false;const x=inbound.get(s.id);return x&&["label_created","in_transit"].includes(String(x.status||""))&&!x.delivered_at;});
    ...
    const shippingCta=document.getElementById("shipping-label-cta");if(shippingCta) shippingCta.hidden=shippingLabelRequired.length===0;
    const deliveryCta=document.getElementById("delivery-cta");if(deliveryCta) deliveryCta.hidden=awaitingDelivery.length===0;
    ...
    const customerCount=document.getElementById("customer-count");if(customerCount) customerCount.textContent=(customers||[]).length;
  }

  await loadCounts();
  setInterval(()=>{if(!document.hidden)loadCounts();},5000);

  // Load the purchasing catalogue hierarchy as a separate controller so the existing
  // purchase pipeline remains isolated from catalogue activation controls.
  const script=document.createElement('script');
  script.src='admin-purchasing-catalog-control.js?v=20260910-hierarchy-1';
  script.defer=true;
