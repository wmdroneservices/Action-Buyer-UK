document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function loadCounts() {
    ...
    const inbound=new Map();(shipments||[]).filter(s=>s.shipment_type==="inbound").forEach(s=>{if(!inbound.has(s.sale_id))inbound.set(s.sale_id,s);});
    ...
    document.getElementById("shipping-label-count").textContent=activeSales.filter(s=>!terminal.has(String(s.status||""))&&s.bank_details_confirmed_at&&!inbound.has(s.id)).length;
    document.getElementById("delivery-count").textContent=activeSales.filter(s=>{if(terminal.has(String(s.status||"")))return false;const x=inbound.get(s.id);return x&&!x.delivered_at&&x.status!=="delivered";}).length;
