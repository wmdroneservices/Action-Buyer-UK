document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function load() {
    ...
    if (assetError || listingError) {
      list.innerHTML = "<p>Could not load sales channels.</p>";
      message.textContent = assetError?.message || listingError?.message || "Could not load sales channels.";
    ...
    const warnings = rows.filter(asset => {
      const assetListings = map.get(asset.id) || [];
      return assetListings.some(x => x.status === "Sold") && assetListings.some(x => activeStatuses.includes(x.status));
    }).length;
    ...
    list.innerHTML = rows.map(asset => {
      const assetListings = map.get(asset.id) || [];
      const active = assetListings.filter(x => activeStatuses.includes(x.status));
      const soldListings = assetListings.filter(x => x.status === "Sold");
      const warning = Boolean(soldListings.length && active.length);
      const cards = channels.map(ch => {
        ...
        const state = l.status === "Sold" ? "SOLD" : l.status === "Cancelled" ? "DELISTED" : l.status;
        return `<span style="display:inline-block;margin:.2rem .35rem .2rem 0;padding:.35rem .55rem;border:1px solid #ddd;border-radius:6px"><strong>${esc(ch)}</strong>: ${esc(state)}${l.asking_price != null ? ` · ${money(l.asking_price)}` : ""}</span>`;
      }).join("");
