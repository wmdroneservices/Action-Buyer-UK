document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function load() {
    ...
    summary.innerHTML=`<div style="display:flex;gap:2rem;flex-wrap:wrap"><div><strong>${rows.length}</strong><br>items sent to Sales</div><div><strong>${published.length}</strong><br>live/reserved listings</div><div><strong>${warnings}</strong><br>delist warnings</div></div><p style="margin-top:1rem">Only staff-sent items appear here. Items in Purchase Inventory are deliberately excluded.</p>`;
    if(!rows.length){list.innerHTML='<div class="empty-account"><h3>No products in Sales Channels</h3><p>Complete inspection and testing, then use Send to Sales from the inventory item.</p></div>';return;}
