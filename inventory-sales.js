document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function load() {
    ...
    summary.innerHTML=`<div style="display:flex;gap:2rem;flex-wrap:wrap"><div><strong>${rows.length}</strong><br>items in Sales</div><div><strong>${readyToList}</strong><br>ready to list for sale</div><div><strong>${published.length}</strong><br>live/reserved listings</div><div><strong>${warnings}</strong><br>delist warnings</div></div><p style="margin-top:1rem">Only staff-sent items appear here. Open the Sales Workbench for the complete marketplace listing process.</p>`;
    if(!rows.length){list.innerHTML='<div class="empty-account"><h3>No products in Sales Channels</h3><p>Complete the product workbench and use Send to Sales. The product will then appear here as <strong>READY TO LIST FOR SALE</strong>.</p></div>';return;}
