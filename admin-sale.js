document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function load() {
    ...
    const outcome = `<section class="account-panel"><div class="section-heading"><p class="section-kicker">FINAL OUTCOME</p><h2>Payment or return</h2><p>Use one of these actions when the sale reaches its final outcome. The customer's order is updated immediately.</p></div><div class="valuation-card"><div style="width:100%;display:grid;gap:1rem;">${paymentSent ? `<p><strong>Payment sent to bank account:</strong> ${esc(payment.reference || "Payment recorded")} ${payment.sentAt ? "· " + dateTime(payment.sentAt) : ""}</p>` : returnSent ? `<p><strong>Return shipped to customer:</strong> ${esc(returns.find(s => s.shipped_at || ["in_transit","delivered"].includes(s.status))?.tracking_number || "Return shipment posted")}</p>` : `<p><strong>Payment:</strong> Not yet recorded as sent.</p><button id="mark-payment-sent" class="btn btn-primary" type="button">PAYMENT SENT TO BANK ACCOUNT</button>`}${!paymentSent && !returnSent ? (returns.length ? `<p><strong>Return:</strong> Return shipment is ready but not yet marked as shipped.</p><button id="mark-return-shipped" class="btn btn-secondary" type="button">RETURN SHIPPED TO CUSTOMER</button>` : "<p><strong>Return:</strong> No return shipment created.</p>") : ""}</div></div></section>`;
