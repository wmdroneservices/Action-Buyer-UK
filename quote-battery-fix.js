...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderBatteryStep() {
    ...
    if (!count) {
      ...
      label.innerHTML = "Number of package batteries being supplied";
    ...
    const previous = Number(count.value);
    count.innerHTML = "";
    for (let i = 0; i <= expected; i++) {
      count.add(new Option(String(i), String(i)));
    }
    count.value = Number.isInteger(previous) && previous >= 0 && previous <= expected ? String(previous) : String(Math.min(expected, 1));
  ...
  function markDefaultPackageContents() {
    const expected = expectedPackageBatteries();
    const count = Number(document.getElementById("package-battery-count")?.value || 0);
    const selects = Array.from(form.querySelectorAll('.package-content-select[data-content-id^="battery-"]'));
    selects.forEach((select, index) => {
      if (index < count) select.value = "present";
      else if (index < expected) select.value = "missing";
    });
  }
  ...
  /* Prevent the core battery creator from leaving a compulsory real battery
     on the page. We replace it with our package-count-controlled UI. */
  const observer = new MutationObserver(function () {
    if (!isDJI()) return;
    const s6 = step6();
    if (s6 && !s6.hidden) renderBatteryStep();
  });
  observer.observe(form, { attributes: true, childList: true, subtree: true, attributeFilter: ["hidden"] });
  ...
  form.addEventListener("click", function (event) {
    ...
    if (button.classList.contains("btn-next")) {
      ...
      /* The core engine requires one entry. For zero supplied batteries the
         hidden marker lets it proceed, while Step 9 records the missing
         package battery and applies the package deduction. */
      if (count === 0) {
        renderBatteryEntries(0);
      }
    }
  }, true);
