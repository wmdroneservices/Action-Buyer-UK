document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function load() {
    const { data: sales, error } = await auth.supabase.from("sales").select("id,sale_reference,status,total_amount,user_id,created_at,accepted_at,bank_details_confirmed_at").order("created_at", { ascending: false });
    ...
    const paymentDueSales = sales.filter(s => {
      ...
      return saleReceived && unpaid && (String(s.status || "") === "payment_due" || Boolean(s.bank_details_confirmed_at));
    });

    const paymentDueBanner = paymentDueSales.length
      ? `<div class="payment-due-alert" style="margin-bottom:1.25rem;padding:14px 16px;border-left:5px solid #c94b2c;background:#fff3ee;color:#8f321f;font-weight:700;border-radius:4px;">ACTION REQUIRED: ${paymentDueSales.length === 1 ? "1 customer has accepted their offer and is due payment." : `${paymentDueSales.length} customers have accepted their offers and are due payment.`}</div>`
      : "";
