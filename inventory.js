document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function load() {
    ...
    list.innerHTML = assets.map(a => `<article class="valuation-card" style="margin-bottom:1rem"><div style="display:flex;justify-content:space-between;gap:1rem;align-items:flex-start;flex-wrap:wrap"><div><p class="section-kicker">${esc(a.status || "Purchased")}</p><h3><a href="inventory-detail.html?id=${encodeURIComponent(a.id)}">${esc([a.manufacturer, a.model].filter(Boolean).join(" ") || "Unnamed asset")}</a></h3><p>Asset: ${esc(a.asset_reference)} · Transaction: ${esc(a.transaction_number || "Not recorded")}</p><p>Serial: ${esc(a.serial_number || "Not recorded")}</p></div><a class="btn btn-secondary" href="inventory-detail.html?id=${encodeURIComponent(a.id)}">VIEW ITEM</a></div><div class="valuation-meta"><p><strong>Purchase cost</strong><br>${money(a.purchase_price)}</p><p><strong>Customer condition</strong><br>${esc(a.customer_condition || "Not recorded")}</p><p><strong>Staff condition</strong><br>${esc(a.condition_grade || "Not recorded")}</p><p><strong>Location</strong><br>${esc(a.current_location || "Not recorded")}</p></div>${a.customer_missing_items ? `<div class="form-message error" style="margin-top:1rem"><strong>Customer reported missing items:</strong> ${esc(a.customer_missing_items_details || "Details not recorded")}</div>` : ""}</article>`).join("");
  }
  await load();
});
