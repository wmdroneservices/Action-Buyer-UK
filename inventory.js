document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function load() {
    ...
    summary.innerHTML = `<div><strong>${active.length}</strong> active asset${active.length === 1 ? "" : "s"}</div><div style="margin-top:.4rem"><strong>${money(totalCost)}</strong> active stock purchase cost</div><div style="margin-top:.4rem"><strong>${assets.filter(a => a.status === "Sold").length}</strong> sold</div>`;
