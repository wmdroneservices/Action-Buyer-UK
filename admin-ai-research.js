...
async function loadResearchPcControl(){
 ...
 const pill=$('rpc-status-pill'); if(pill){pill.textContent=fresh?'ONLINE':'OFFLINE';pill.className='rpc-status '+(fresh?'online':'offline');}
 if($('rpc-worker'))$('rpc-worker').textContent=fresh?(a.status||'ONLINE').toUpperCase():'OFFLINE';
 if($('rpc-ollama'))$('rpc-ollama').textContent=fresh?'CONNECTED':'CHECK REQUIRED';
