/* Multi-item quote wizard: reset every item field and keep each item's photos separate. */
(function(){
  "use strict";
  ...
  const values=selector=>Array.from(form.querySelectorAll(selector)).map(el=>el.value);
  ...
  function collectItem(){
    const q={
      category:clean(document.getElementById("gear-category")?.value),categoryName:selectedText("gear-category"),
      manufacturer:clean(document.getElementById("gear-manufacturer")?.value),manufacturerName:selectedText("gear-manufacturer"),
      model:clean(document.getElementById("dji-model")?.value),modelName:selectedText("dji-model"),
      package:clean(document.getElementById("package-select")?.value),packageName:selectedText("package-select"),
      condition:checked("condition"),flightHours:clean(document.getElementById("flight-hours")?.value),flightHoursRange:checked("flightHoursRange"),
      ...
      unbound:checked("unbound"),damage:checked("damage"),damageDescription:clean(document.getElementById("damage-description")?.value),
      ...
      droneSerial:clean(document.getElementById("drone-serial-number")?.value),controllerSerial:clean(document.getElementById("controller-serial-number")?.value),
      legalRight:checked("legalRight"),valuation:"manual",amount:null
    };
  ...
  function clearAllFields(){
    ...
    const damage=document.getElementById("damage-details");if(damage)damage.hidden=true;
    const ownership=document.getElementById("ownership-warning");if(ownership)ownership.hidden=true;
    const unbound=document.getElementById("unbound-warning");if(unbound)unbound.hidden=true;
    const usage=document.getElementById("gear-usage-count-wrap");if(usage)usage.hidden=true;
    if(window.__gcoMultiItemFiles)window.__gcoCurrentItemIndex=null;
  ...
  window.gearCashOutResetForNewItem=clearAllFields;
})();
