...
(function(){
  ...
  function quoteRecord(userId,photoData,reference,items){
    const title=document.getElementById("quote-result-title")?.textContent||"",manual=/manual valuation|manual validation/i.test(title),value=id=>clean(document.getElementById(id)?.value),category=document.getElementById("gear-category"),manufacturer=document.getElementById("gear-manufacturer"),model=document.getElementById("dji-model"),packageSelect=document.getElementById("package-select");
  ...
  async function submitAuthenticatedQuote(){
    ...
    try{const reference="WBA-"+new Date().getFullYear()+"-"+Math.floor(100000+Math.random()*900000),photoData=await uploadPhotos(session.user.id,reference),record=quoteRecord(session.user.id,photoData,reference,items),saved=await auth.supabase.from("valuations").upsert(record,{onConflict:"quote_reference"});if(saved.error)throw saved.error;const{data:valuation,error:valuationError}=await auth.supabase.from("valuations").select("id").eq("quote_reference",reference).maybeSingle();if(valuationError||!valuation)throw valuationError||new Error("Could not locate submitted quote.");await syncItems(valuation.id,record.item_records);try{localStorage.setItem("wba_latest_quote",JSON.stringify(record.quote_data));localStorage.removeItem("gearCashOutQuoteBasket")}catch(_){}if(Array.isArray(window.__gcoMultiItemFiles))window.__gcoMultiItemFiles=[];showSubmitted(record)}catch(error){console.error("Could not submit batched quote:",error);alert(error?.message||"The quote could not be submitted. Please try again.")}finally{busy=false;if(button)button.disabled=false}
  }
