...
(function(){
  ...
  async function submitAuthenticatedQuote(){
    ...
    if(items.length>1 && (groups.length!==items.length || groups.some(group=>!group.length))){alert("Each item in a multi-item quote must have at least one photograph. Please return to the item list and add the missing photographs before submitting.");return;}
