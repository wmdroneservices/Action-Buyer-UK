...
(function () {
  ...
  let busy = false;
  const money = text => { const match = String(text || "").replace(/,/g, "").match(/£\s*([0-9]+(?:\.[0-9]+)?)/); return match ? Number(match[1]) : null; };
  ...
  function quoteRecord(userId, photos, reference) {
    ...
    const batteries = Array.from(document.querySelectorAll(".battery-entry")).map(row => ({ type: clean(row.querySelector(".battery-type")?.value), cycles: Number(row.querySelector(".battery-cycles")?.value || 0) }));
    const contents = {};
    ...
    const amount = manual ? null : money(document.querySelector("#quote-summary .quote-price")?.textContent);
    return {
      user_id: userId,
      ...
      quote_amount: amount,
      quote_data: { category: category?.value || "", categoryName: selectedText("gear-category"), manufacturer: manufacturer?.value || "", manufacturerName: selectedText("gear-manufacturer"), model: model?.value || "", modelName: selectedText("dji-model"), package: packageSelect?.value || "", packageName: selectedText("package-select"), condition: checked("condition"), flightHours: value("flight-hours"), flightHoursRange: checked("flightHoursRange"), batteries, unbound: checked("unbound"), damage: checked("damage"), damageDescription: value("damage-description"), packageContents: contents, additionalAccessories: [], droneSerial: value("drone-serial-number"), controllerSerial: value("controller-serial-number"), photos, legalRight: checked("legalRight"), fullName: value("full-name"), email: value("email-address"), phone: value("phone-number"), addressLine1: value("address-line-1"), addressLine2: value("address-line-2"), city: value("city"), county: value("county"), postcode: value("postcode").toUpperCase(), quoteAmount: amount, quoteReference: reference, photosProvided: photos.length > 0, resumedAfterLogin: false, created: new Date().toISOString() }
    };
  }

  async function uploadPhotos(userId, reference) {
    const files = Array.from(document.getElementById("photo-uploads")?.files || []);
    if (!files.length) return [];
    const uploaded = [];
    for (let i = 0; i < files.length; i += 1) {
      const file = files[i];
      const ext = (file.name.split(".").pop() || "jpg").toLowerCase().replace(/[^a-z0-9]/g, "") || "jpg";
      const path = `${userId}/${reference}/${Date.now()}-${i}.${ext}`;
  ...
  function showSubmitted(record) {
    document.querySelectorAll("#quote-form .wizard-step").forEach(step => { step.hidden = Number(step.dataset.step) !== 14; });
  ...
  async function submitAuthenticatedQuote() {
    ...
    if (!session) return;
    const required = [["full-name", "Please enter your full name."], ["email-address", "Please enter your email address."], ["phone-number", "Please enter your telephone number."]];
    for (const [id, error] of required) { if (!clean(document.getElementById(id)?.value)) { alert(error); return; } }
    busy = true;
    ...
    try {
      ...
      const record = quoteRecord(session.user.id, photos, reference);
      const { error } = await auth.supabase.from("valuations").upsert(record, { onConflict: "quote_reference" });
      if (error) throw error;
      try { localStorage.setItem("wba_latest_quote", JSON.stringify(record.quote_data)); } catch (_) {}
  ...
  document.addEventListener("click", async event => {
    ...
    let resume = null;
    try { resume = JSON.parse(localStorage.getItem("gearCashOutQuoteResume") || "null"); } catch (_) {}
    if (resume) return;
