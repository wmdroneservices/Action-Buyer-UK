alter table public.inventory_assets
  add column if not exists items_added_replaced text,
  add column if not exists final_package_contents text,
...
set customer_condition=coalesce(ia.customer_condition,
    ...
    customer_missing_items_details=coalesce(ia.customer_missing_items_details,
      nullif(trim(coalesce(qi.item_data->>'missingItemsDetails','')), ''),
      nullif(trim(coalesce(qi.item_data->>'exceptionNotes','')), '')),
...
create or replace function public.set_inventory_testing_updated_at()
returns trigger
language plpgsql
...
revoke execute on function public.handle_resale_listing_sold() from public, anon, authenticated;
revoke execute on function public.sync_inventory_asset_when_resale_listing_sold() from public, anon, authenticated;
revoke execute on function public.sync_inventory_customer_details() from public, anon, authenticated;
revoke execute on function public.set_inventory_testing_updated_at() from public, anon, authenticated;
