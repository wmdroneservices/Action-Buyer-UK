...
declare q jsonb; c text; valuation_condition text; missing boolean; damage boolean; details text;
begin
  if new.source_quote_item_id is null then return new; end if;
  select qi.item_data, v.condition into q, valuation_condition
...
set customer_condition=coalesce(ia.customer_condition,nullif(trim(coalesce(qi.item_data->>'condition','')), ''),nullif(trim(coalesce(qi.item_data->'singleItem'->>'condition','')), ''),nullif(trim(coalesce(v.condition,'')), '')),
