create table if not exists public.quote_item_refusals (
  id uuid primary key default gen_random_uuid(),
  item_id uuid not null references public.quote_items(id) on delete cascade,
  offer_id uuid references public.quote_offers(id) on delete set null,
  reason text,
  refused_by uuid not null references auth.users(id),
  refused_at timestamptz not null default now()
);

alter table public.quote_item_refusals enable row level security;

drop policy if exists "Staff can manage item refusals" on public.quote_item_refusals;
create policy "Staff can manage item refusals"
  on public.quote_item_refusals
  for all
  to authenticated
...
revoke all on public.quote_item_refusals from anon, authenticated;
grant select, insert, update, delete on public.quote_item_refusals to authenticated;

create index if not exists quote_item_refusals_item_id_idx on public.quote_item_refusals(item_id);
create index if not exists quote_item_refusals_refused_at_idx on public.quote_item_refusals(refused_at desc);
