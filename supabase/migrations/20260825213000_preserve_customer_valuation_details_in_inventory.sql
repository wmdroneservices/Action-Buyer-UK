-- Preserve customer-selected valuation details when a paid sale becomes inventory.
...
begin
  ...
  loop
    v_raw_condition := nullif(trim(coalesce(v_item.valuation_condition,'')), '');
    if v_raw_condition is null then v_raw_condition := nullif(trim(coalesce(v_item.item_data->>'condition','')), ''); end if;
    if v_raw_condition is null then v_raw_condition := nullif(trim(coalesce(v_item.item_data->'singleItem'->>'condition','')), ''); end if;
    v_condition := case lower(v_raw_condition)
      when 'factory-sealed' then 'Factory Sealed / Unopened'
      when 'opened-unused' then 'Opened but Unused'
      when 'excellent' then 'Excellent'
      when 'good' then 'Good'
      when 'fair' then 'Fair'
      when 'damaged' then 'Damaged'
      when 'not-working' then 'Not Working / Spares Only'
      else v_raw_condition
    end;
...
set condition_grade=coalesce(
      ia.condition_grade,
      case lower(coalesce(
        nullif(trim((select qi.item_data->>'condition' from public.quote_items qi where qi.id=ia.source_quote_item_id)),''),
        nullif(trim((select qi.item_data->'singleItem'->>'condition' from public.quote_items qi where qi.id=ia.source_quote_item_id)),''),
        nullif(trim((select v.condition from public.quote_items qi left join public.valuations v on v.id=qi.valuation_id where qi.id=ia.source_quote_item_id)),''),
        ''
      ))
        when 'factory-sealed' then 'Factory Sealed / Unopened'
        when 'opened-unused' then 'Opened but Unused'
