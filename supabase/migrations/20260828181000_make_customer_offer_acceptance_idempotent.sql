-- GearCashOut: make customer offer acceptance idempotent.
-- The account page refreshes offer cards periodically. If a second request
-- reaches the RPC after the first request has already accepted the offer,
-- treat that second request as a successful no-op rather than showing the
-- customer "Offer is not available for acceptance".
...
begin
  ...
  -- Accepting the same offer twice is harmless. This also handles a race
  -- where the account page replaces the button while the first request is
  -- still completing.
  ...
  join public.valuations v on v.id=qi.valuation_id
  where qi.id=v_offer.item_id and v.user_id=v_user;
  if not found then
    raise exception 'Offer is not available for acceptance';
  end if;
  ...
  where qo.id=p_offer_id and v.user_id=v_user
  for update;
  if not found or v_offer.status not in ('published','accepted') then
    raise exception 'Offer is not available for acceptance';
  end if;
