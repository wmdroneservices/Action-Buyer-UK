-- In a combined submission, an item may be accepted or refused only after
-- every other still-pending item has an effective published price.
-- Once another item is already accepted/refused/closed, it no longer blocks
-- the customer's decision on this item.
...
begin
 ...
 if nullif(trim(v_submission_key),'') is not null and exists (
  select 1 from public.quote_items qi2 join public.valuations v2 on v2.id=qi2.valuation_id
  where v2.user_id=v_user and v2.quote_data->>'submissionKey'=v_submission_key and qi2.item_status not in ('refused','closed','accepted')
 ...
 ) then raise exception 'This is a combined valuation. Please wait until all item prices have been reviewed and published before accepting or refusing an item.'; end if;
...
begin
 ...
 if exists(select 1 from public.quote_offers where item_id=v_offer.item_id and status='accepted') then v_item_status:='accepted'; elsif exists(select 1 from public.quote_offers where item_id=v_offer.item_id and status='published') then v_item_status:='under_assessment'; else v_item_status:='refused'; end if;
