...
async function manager(req:Request){
 ...
 const admin=createClient(url,service),{data:staff}=await admin.from("staff_users").select("active,can_manage_staff").eq("user_id",user.id).maybeSingle();
 if(!staff?.active||!staff.can_manage_staff)throw Error("Management access required");
 return {admin,user};
}
...
async function boxes(admin:any){
 ...
 const primary=defaultBox(),rows=[primary,...(data||[]).filter(x=>x.email_address!==primary.email_address)];
 return rows.map(x=>{const c=cfg(x);return {...x,smtpConfigured:!!(c.smtpUser&&c.smtpPass),imapConfigured:!!(c.imapUser&&c.imapPass)}});
}
async function boxFor(admin:any,id:string){const all=await boxes(admin);const box=all.find(x=>String(x.id)===String(id||"default"));if(!box)throw Error("Mailbox not found");return box;}

Deno.serve(async req=>{
 if(req.method==="OPTIONS")return new Response("ok",{headers:cors});
 try{
  const {admin}=await manager(req),body=await req.json().catch(()=>({})),action=String(body.action||"status");
  if(action==="status"||action==="mailboxes")return json({ok:true,mailboxes:await boxes(admin)});
  if(action==="staff"){const {data,error}=await admin.from("staff_users").select("user_id,display_name,username,business_email,active").eq("active",true).order("display_name");if(error)throw error;return json({ok:true,staff:data||[]});}
  ...
  const box=await boxFor(admin,String(body.mailbox_id||"default")),c=cfg(box);
