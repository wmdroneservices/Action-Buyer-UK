...
// Keeps the existing sale page intact while enforcing the post-receipt sequence in the UI.
(async () => {
  ...
  async function getSale() {
    const {data,error}=await auth.supabase.from('sales').select('*').eq('id',saleId).maybeSingle();
    return error ? null : data;
  }

  async function addCommunications(sale) {
  ...
  async function confirmPayment(button, referenceInput) {
    ...
    button.disabled=true;
    const {data,error}=await auth.supabase.rpc('staff_mark_sale_paid_and_create_inventory',{p_sale_id:saleId,p_payment_reference:reference});
  ...
  function installCapture() {
    document.addEventListener('click', event => {
      ...
      if(paymentButton){
        ...
        const input=document.getElementById('workflow-payment-reference');
        confirmPayment(paymentButton,input);
      }
    }, true);
  }
  ...
  if(!sale) return;
  installCapture();
  await addCommunications(sale);

  const box=document.getElementById('sale-details');
  if(!box) return;
  const existing=box.querySelector('.workflow-next-step');
