...
function setAuthMarker(session) {
  try {
    if (session) localStorage.setItem("gearCashOutAuthenticated", "true");
    else localStorage.removeItem("gearCashOutAuthenticated");
  } catch (_) {}
}
...
window.actionBuyerAuth = {
  ...
  async getSession() {
    ...
    setAuthMarker(data?.session || null);
  ...
  async getProfile() {
    ...
    const { data, error } = await supabaseClient.from("profiles").select("full_name, phone, address_line1, address_line2, city, county, postcode").eq("id", session.user.id).maybeSingle();
    if (error) { console.error("Could not load customer profile:", error); return null; }
    return data || null;
  },
  async prefillQuoteCustomerDetails() {
    const profile = await this.getProfile();
    if (!profile) return;
    const fields = { "full-name": profile.full_name, "phone-number": profile.phone, "address-line-1": profile.address_line1, "address-line-2": profile.address_line2, "city": profile.city, "county": profile.county, "postcode": profile.postcode };
    Object.entries(fields).forEach(([id, value]) => { const field = document.getElementById(id); if (field && value && !field.value) field.value = value; });
    const email = document.getElementById("email-address");
    const session = await this.getSession();
    if (email && session?.user?.email && !email.value) email.value = session.user.email;

    const addressFieldset = document.querySelector('#quote-form .wizard-step[data-step="13"] fieldset');
    const addressInputs = document.querySelectorAll('#quote-form .wizard-step[data-step="13"] #address-line-1, #quote-form .wizard-step[data-step="13"] #address-line-2, #quote-form .wizard-step[data-step="13"] #city, #quote-form .wizard-step[data-step="13"] #county, #quote-form .wizard-step[data-step="13"] #postcode');
    addressInputs.forEach((input) => { input.required = false; });
    if (addressFieldset) addressFieldset.hidden = true;
    const detailsStep = document.querySelector('#quote-form .wizard-step[data-step="13"]');
    ...
    if (detailsStep && !addressNotice) {
      ...
      notice.innerHTML = "<strong>Return address saved.</strong> We already have your return address on your account. It is not required again at this stage.";
      const firstField = detailsStep.querySelector("#full-name");
      if (firstField?.parentNode) firstField.parentNode.insertBefore(notice, firstField);
    }
  },
  async saveQuoteCustomerDetails() {
    const session = await this.getSession();
    if (!session) return;
    const value = (id) => { const field = document.getElementById(id); return field ? field.value.trim() : ""; };
    const record = { id: session.user.id, full_name: value("full-name"), phone: value("phone-number"), address_line1: value("address-line-1"), address_line2: value("address-line-2"), city: value("city"), county: value("county"), postcode: value("postcode"), updated_at: new Date().toISOString() };
    const existing = await this.getProfile();
    if (existing) {
      ["address_line1", "address_line2", "city", "county", "postcode"].forEach((key) => { if (!record[key] && existing[key]) record[key] = existing[key]; });
    ...
    const { error } = await supabaseClient.from("profiles").upsert(record, { onConflict: "id" });
    if (error) console.error("Could not save customer profile:", error);
  }
};
