...
(function(){
  ...
  function ensureOfficialLink(card,id){
    ...
    if(!link){
      link=document.createElement('a');link.className='catalog-official-manufacturer-link';link.target='_blank';link.rel='noopener noreferrer';link.textContent='OPEN DJI OFFICIAL PRODUCT PAGE ↗';link.style.cssText='display:inline-block;margin-top:.25rem;font-size:.72rem;font-weight:700;color:#102f4f;text-decoration:underline;';title.appendChild(link);
    }
  ...
  function setPrice(card){
    ...
    const existing=p.dataset.catalogConditions;let conditions=existing;
    if(!conditions){conditions=(p.textContent||'').replace(/^\s*(?:Online comparison\s+(?:£[\d,]+(?:\.\d{2})?(?:–£[\d,]+(?:\.\d{2})?)?|—)|RRP\s+[^·]+)\s*·\s*/i,'').trim();p.dataset.catalogConditions=conditions;}
    const prices=cache.get(id)||[];const unique=[...new Set(prices)].sort((a,b)=>a-b);
    ...
    if(p.dataset.onlineComparisonRendered!==desired){p.innerHTML=`<span class="catalog-online-comparison-text">${esc(label)}</span> · ${esc(conditions)}`;p.dataset.onlineComparisonRendered=desired;}
  ...
  async function fetchPrices(){
    ...
    while(true){
      ...
      (data||[]).forEach(row=>{if(!isDirectRetail(row))return;const price=Number(row.sell_price);if(!Number.isFinite(price)||price<=0||!row.catalog_product_id)return;if(!next.has(row.catalog_product_id))next.set(row.catalog_product_id,[]);next.get(row.catalog_product_id).push(price);});
  ...
  async function fetchOfficialLinks(){
    ...
    while(true){
      ...
      (data||[]).forEach(row=>{const url=String(row.source_url||'');if(!/^https?:\/\/(?:store\.)?dji\.com\//i.test(url))return;if(!row.catalog_product_id)return;const current=next.get(row.catalog_product_id)||'';const score=u=>/store\.dji\.com\/uk\/product\//i.test(u)?3:/www\.dji\.com\/uk\//i.test(u)?2:/store\.dji\.com\/uk\//i.test(u)?1:0;if(!current||score(url)>score(current))next.set(row.catalog_product_id,url);});
      if(!data||data.length<pageSize)break;from+=pageSize;
    }
    officialLinks.clear();next.forEach((v,k)=>officialLinks.set(k,v));renderAll();
  }
  function wire(){const list=document.getElementById('catalog-list');if(!list||wired)return;wired=true;const observer=new MutationObserver(()=>renderAll());observer.observe(list,{childList:true});renderAll();fetchPrices();fetchOfficialLinks();}
...
(function(){
  ...
  function enhanceRow(tr){if(!tr||tr.dataset.competitorEnhanced==='1')return;const input=tr.querySelector('.retailer');if(!input)return;const current=(input.value||'').trim();const select=document.createElement('select');select.className='retailer competitor-select';select.setAttribute('aria-label','Website / Competitor');const known=websites.map(([name])=>name);if(current&&!known.some(name=>name.toLowerCase()===current.toLowerCase())){const existing=document.createElement('option');existing.value=current;existing.textContent=current;select.appendChild(existing);}websites.forEach(([name])=>{const option=document.createElement('option');option.value=name;option.textContent=name;select.appendChild(option);});const match=known.find(name=>name.toLowerCase()===current.toLowerCase());select.value=match||current||'Custom / Other';input.replaceWith(select);const link=document.createElement('a');link.className='retailer-link competitor-website-link';link.target='_blank';link.rel='noopener noreferrer';link.textContent='OPEN WEBSITE ↗';select.closest('td').appendChild(link);const sourceInput=tr.querySelector('.retailer-source');function updateLink(){const canonical=urlMap.get(select.value.toLowerCase())||'';const source=safeUrl(sourceInput?.value?.trim());const isCustom=!canonical&&select.value.toLowerCase()==='custom / other';const url=canonical||(isCustom?source:'');if(url){link.href=url;link.classList.remove('disabled');link.title=canonical?'Open the competitor website':'Open the recorded source page';}else{link.removeAttribute('href');link.classList.add('disabled');link.title='Enter a source URL for this competitor';}}select.addEventListener('change',updateLink);sourceInput?.addEventListener('input',updateLink);updateLink();tr.dataset.competitorEnhanced='1';}
